...
#新增
from .DSSABlock import DropPath,MixFFN,DeformMixFFN,DeformMambaEncoderLayer
...
class Stem(BaseModule):
    ...
    def __init__(
        ...
        self.down_conv = ConvModule(in_channels, hidden_channels, kernel_size=3, stride=1, padding=1,
                                    norm_cfg=norm_cfg, act_cfg=act_cfg)
...
class DownSamplingLayer(BaseModule):
    ...
    def __init__(
        ...
        self.down_conv = ConvModule(in_channels, out_channels, kernel_size=3, stride=1, padding=1,
                                    norm_cfg=norm_cfg, act_cfg=act_cfg)
...
class PKIBlock(BaseModule):
    """Poly Kernel Inception Block followed by DSSA Block"""
    def __init__(
            self,
            in_channels: int,
            out_channels: Optional[int] = None,
            ...
            act_cfg: Optional[dict] = dict(type='SiLU'),
            # DSSA Block 参数
            enable_dssa: bool = True,
            dssa_feedforward_channels: Optional[int] = None,
            dssa_drop_rate: float = 0.2,
            dssa_drop_path_rate: float = 0.2,
            dssa_proj_drop: float = 0.2,
            init_cfg: Optional[dict] = None,
    ):
        super().__init__(init_cfg)
        out_channels = out_channels or in_channels
        ...
        self.block = InceptionBottleneck(
            ...
            norm_cfg=norm_cfg, act_cfg=act_cfg)
        self.ffn = ConvFFN(
            hidden_channels, out_channels, ffn_scale, ffn_kernel_size, dropout_rate,
            add_identity=False, norm_cfg=None, act_cfg=None)

        # DSSA Block 部分
        self.enable_dssa = enable_dssa
        if self.enable_dssa:
            dssa_feedforward_channels = dssa_feedforward_channels or int(out_channels * ffn_scale)
            self.dssa_block = DeformMambaEncoderLayer(
                embed_dims=out_channels,
                feedforward_channels=dssa_feedforward_channels,
                drop_rate=dssa_drop_rate,
                drop_path_rate=dssa_drop_path_rate,
                proj_drop=dssa_proj_drop,
                dropout_layer=dict(type='Dropout', drop_prob=dssa_drop_path_rate))
    ...
    def forward(self, x):
        ...
        if self.enable_dssa:
            ...
            x_nlc = nchw_to_nlc(x)
            x_nlc = self.dssa_block(x_nlc, hw_shape)
            x = nlc_to_nchw(x_nlc, hw_shape)
    ...
    def init_weights(self):
        super().init_weights()
        if self.enable_dssa:
            for m in self.dssa_block.modules():
...
class PKIStage(BaseModule):
    ...
    def __init__(
            ...
            drop_path_rate: Union[float, list] = 0.1,
            layer_scale: Optional[float] = 1.0,
            ...
            with_caa: bool = True,
            caa_kernel_size: int = 11,
            enable_dssa: bool = False,               # 新增参数
            dssa_feedforward_channels: int = 256,    # 新增参数
            dssa_drop_rate: float = 0.1,             # 新增参数
            dssa_drop_path_rate: float = 0.1,        # 新增参数
            dssa_proj_drop: float = 0.1,             # 新增参数
            norm_cfg: Optional[dict] = dict(type='BN', momentum=0.03, eps=0.001),
            act_cfg: Optional[dict] = dict(type='SiLU'),
            init_cfg: Optional[dict] = None,
    ):
...
class PKINet(BaseModule):
    ...
    arch_settings = {
        ...
        #enable_dssa,dssa_feedforward_channels（DSSA 前馈通道数）,dssa_drop_rate（丢弃率）,dssa_drop_path_rate（路径丢弃率）,dssa_proj_drop（投影丢弃率）
        'T': [[16, 32, 4, (3, 5, 7, 9, 11), (1, 1, 1, 1, 1), 0.5, 4.0, 3, 0.1, 1.0, True, 8.0, 5, True, True, 11, False, None, 0.2, 0.2, 0.2],
              [32, 64, 14, (3, 5, 7, 9, 11), (1, 1, 1, 1, 1), 0.5, 4.0, 3, 0.1, 1.0, True, 8.0, 7, True, True, 11, False, None, 0.2, 0.2, 0.2],
              [64, 128, 22, (3, 5, 7, 9, 11), (1, 1, 1, 1, 1), 0.5, 4.0, 3, 0.1, 1.0, True, 4.0, 9, True, True, 11, True, 256, 0.2, 0.2, 0.2],
              [128, 256, 4, (3, 5, 7, 9, 11), (1, 1, 1, 1, 1), 0.5, 4.0, 3, 0.1, 1.0, True, 4.0, 11, True, True, 11, True, 512, 0.2, 0.2, 0.2]],
        'S': [[32, 64, 4, (3, 5, 7, 9, 11), (1, 1, 1, 1, 1), 0.5, 4.0, 3, 0.1, 1.0, True, 8.0, 5, True, True, 11, False, None, 0.2, 0.2, 0.2],
              [64, 128, 12, (3, 5, 7, 9, 11), (1, 1, 1, 1, 1), 0.5, 4.0, 3, 0.1, 1.0, True, 8.0, 7, True, True, 11, False, None, 0.2, 0.2, 0.2],
              [128, 256, 20, (3, 5, 7, 9, 11), (1, 1, 1, 1, 1), 0.5, 4.0, 3, 0.1, 1.0, True, 4.0, 9, True, True, 11, True, 256, 0.2, 0.2, 0.2],
              [256, 512, 4, (3, 5, 7, 9, 11), (1, 1, 1, 1, 1), 0.5, 4.0, 3, 0.1, 1.0, True, 4.0, 11, True, True, 11, True, 512, 0.2, 0.2, 0.2]],
        'B': [[40, 80, 6, (3, 5, 7, 9, 11), (1, 1, 1, 1, 1), 0.5, 4.0, 3, 0.1, 1.0, True, 8.0, 5, True, True, 11, False, None, 0.2, 0.2, 0.2],
              [80, 160, 16, (3, 5, 7, 9, 11), (1, 1, 1, 1, 1), 0.5, 4.0, 3, 0.1, 1.0, True, 8.0, 7, True, True, 11, True, 320, 0.2, 0.2, 0.2],
              [160, 320, 24, (3, 5, 7, 9, 11), (1, 1, 1, 1, 1), 0.5, 4.0, 3, 0.1, 1.0, True, 4.0, 9, True, True, 11, True, 640, 0.2, 0.2, 0.2],
              [320, 640, 6, (3, 5, 7, 9, 11), (1, 1, 1, 1, 1), 0.5, 4.0, 3, 0.1, 1.0, True, 4.0, 11, True, True, 11, True, 1280, 0.2, 0.2, 0.2]],
}
    ...
    def __init__(
        ...
        for i, (in_channels, out_channels, num_blocks, kernel_sizes, dilations, expansion, ffn_scale, ffn_kernel_size,
                ...
                add_identity, with_caa, caa_kernel_size,enable_dssa, dssa_feedforward_channels, dssa_drop_rate,
                dssa_drop_path_rate, dssa_proj_drop) in enumerate(arch_setting):
            stage = PKIStage(in_channels, out_channels, num_blocks, kernel_sizes, dilations, expansion,
                             ...
                             add_identity, with_caa, caa_kernel_size, enable_dssa, dssa_feedforward_channels, dssa_drop_rate, dssa_drop_path_rate, dssa_proj_drop,norm_cfg, act_cfg)
