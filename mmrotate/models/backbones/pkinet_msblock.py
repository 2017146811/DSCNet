...
from .MS2D import MSBlock
from .ARConv import ARConv
# from DCNv4_op.DCNv4.modules.dcnv4 import DCNv4
# from .DFormerv2 import RGBD_Block
# from .SCSA import SANet


# pki_caaGCSA_inception_VSS
...
#                                  (v_kernel_size // 2, 0), groups=channels,
#                                  norm_cfg=None, act_cfg=None)
#         self.gcsa = GCSA(dim=channels, num_heads=num_heads, bias=bias)
#
#         # self.rgbd_block1 = RGBD_Block(
#         #         split_or_not=True,
#         #         embed_dim=channels,
#         #         num_heads=num_heads,
#         #         ffn_dim=channels * 2,
...
#         # )
#         self.scsa = SANet(in_planes=channels)
...
#         # #rgbd_block
#         # if x_e is None:
#         #     x_e = x.mean(dim=1, keepdim=True)
#         #     # 将 x 转置为 [B, H, W, C] 以适配 RGBD_Block
#         # x = x.permute(0, 2, 3, 1)  # [B, H, W, C]
#         #     # 调用 RGBD_Block
#         # out = self.rgbd_block1(x, x_e, split_or_not=True)  # [B, H, W, C]
#         #     # 将输出转置回 [B, C, H, W]
#         # out = out.permute(0, 3, 1, 2)  # [B, C, H, W]
#     def forward(self, x):
#         # SCSA
#         assert len(x.shape) == 4, f"预期输入为 4D 张量，实际为 {x.shape}"
#         B, C, H, W = x.shape
#         self.logger.debug(f"CAA 输入形状: {x.shape}")
#
#         content = style = x
#         content_sem = style_sem = x
#         sem_map = torch.ones((B, 1, H * W), device=x.device)
#
#         out = self.scsa(content, style, content_sem, style_sem, sem_map, sem_map)
#         # B, C, H, W = x.shape
#         # sem_map_32 = torch.ones((B, 1, H * W), device=x.device)
#         # sem_map_64 = torch.ones((B, 1, H * W), device=x.device)
#         # # 使用 x 本身作为 content 和 style
#         # content = style = x
#         # content_sem = style_sem = x.mean(dim=1, keepdim=True)
#         # # 调用 SCSA 模块（参数 t1, t2 可调节风格/细节权重）
#         # out = self.scsa(content, style, content_sem, style_sem, sem_map_32, sem_map_64)
...
class GCSA(nn.Module):
    ...
    def __init__(self, dim, num_heads, bias):
        ...
        self.qkv_dwconv1 = nn.Conv2d(dim * 3, dim * 3, kernel_size=3, padding=1, groups=dim * 3, bias=bias)  # 普通卷积

        self.qkv_dwconv2 = nn.Conv2d(dim * 3, dim * 3, kernel_size=3, stride=1, dilation=2, padding=2, groups=dim * 3,
                                     bias=bias)
        # 融合分支
        self.qkv_fusion = nn.Conv2d(dim * 6, dim * 3, kernel_size=1, bias=bias)

        self.qkv = nn.Conv2d(dim, dim * 3, kernel_size=1, bias=bias)

        self.project_out = nn.Conv2d(dim, dim, kernel_size=1, bias=bias)
        self.temperature = nn.Parameter(torch.ones(num_heads, 1, 1))
        # 用于融合傅里叶增强特征（如 concat 后 2x 通道）
        # self.fourier_fusion = nn.Conv2d(dim * 2, dim, kernel_size=1, bias=bias)
    ...
    def forward(self, x):
        ...
        # 两种感受野卷积
        qkv_local = self.qkv_dwconv1(qkv)  # 普通卷积增强
        qkv_global = self.qkv_dwconv2(qkv)  # 膨胀卷积增强

        # 拼接 + 融合
        qkv = torch.cat([qkv_local, qkv_global], dim=1)  # (B, 6C, H, W)
        qkv = self.qkv_fusion(qkv)  # (B, 3C, H, W)

        q, k, v = qkv.chunk(3, dim=1)

        # ===== ⭐ 添加傅里叶增强通道（在 V 上）=====
        # v_fft = torch.fft.fft2(v, norm='ortho')          # [b, c, h, w]
        # v_fft_mag = torch.abs(v_fft)                     # 幅度谱 [b, c, h, w]
        # v_fused = self.fourier_fusion(torch.cat([v, v_fft_mag], dim=1))  # [b, c, h, w]
        ...
        # v = rearrange(v_fused, 'b (head c) h w -> b head c (h w)', head=self.num_heads)
        # v = rearrange(v, 'b (head c) h w -> b head c (h w)', head=self.num_heads)
...
class InceptionBottleneck(BaseModule):
    ...
    def __init__(
        ...
        self.msblock = MSBlock(hidden_dim=hidden_channels, drop_path=drop_path,expand=expansion)

        self.gate_ms = nn.Sequential(
    ...
    def forward(self, x):
        ...
        y = x
        x = self.pre_conv(x)
        x_v = self.msblock(x.permute(0, 2, 3, 1))
        # x_v = x_v.permute(0, 3, 1, 2)
        # print(f"After permute: x_v.shape = {x_v.shape}")
        x = x + self.gate_ms(x) * x_v
        if self.caa_factor is not None:
            y = self.caa_factor(self.pre_conv(y))
        if self.add_identity:
            gate_weight = self.gate(x)
            x = gate_weight * x + (1 - gate_weight) * y
        else:
            x = x * y
        x = self.post_conv(x)
        return x
...
#         x = self.pw_conv(x)
#         # x = self.pre_conv(x)  # BCHW
#         # y = x

#         # VSS -> EFConv -> KANBlock
#         x = x.permute(0, 2, 3, 1)  # BCHW -> BHWC
#         x = x + self.vss(self.ln(x))  # VSS (BHWC)
#         B, H, W, C = x.shape
#         x = self.ln(x)

#         x = x.permute(0, 3, 1, 2)  # BHWC -> BCHW for EFConv
#         x, H, W = self.EFConv(x)
...
class PKINet(BaseModule):
    ...
    arch_settings = {
        ...
        'S': [[32, 64, 2, (3, 5, 7, 9, 11), (1, 1, 1, 1, 1), 0.5, 4.0, 3, 0.1, 1.0, True, 8.0, 5, True, True, 11],
              [64, 128, 6, (3, 5, 7, 9, 11), (1, 1, 1, 1, 1), 0.5, 4.0, 3, 0.1, 1.0, True, 8.0, 7, True, True, 11],
              [128, 256, 10, (3, 5, 7, 9, 11), (1, 1, 1, 1, 1), 0.5, 4.0, 3, 0.1, 1.0, True, 4.0, 9, True, True, 11],
