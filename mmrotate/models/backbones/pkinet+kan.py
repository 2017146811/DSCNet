...
class ConvFFN(BaseModule):
    ...
    def __init__(
        ...
        self.ffn_layers = nn.Sequential(
            ...
            # GCSA(dim=hidden_channels, num_heads=num_heads, bias=bias) , # 插入注意力模块
...
class KANBlock(nn.Module):
    def __init__(
        ...
        self.kan = KAN(
            layers_hidden=[in_dim] + hidden_dims + [out_dim],
            grid_size=grid_size,
    ...
    def forward(self, x, update_grid=False):
        """
        Args:
            x: Tensor of shape [B, N, C] or [B, C] (depending on input format).
            update_grid: Whether to update grid adaptively (during training or fine-tuning).
        """
        identity = x
        x = self.norm(x)
...
class InceptionBottleneck(BaseModule):
    ...
    def __init__(
        ...
        self.VSS = VSSBlock(hidden_dim=hidden_channels, depth=1, drop_path=drop_path)
    ...
    def forward(self, x):
        ...
        x = self.pre_conv(x)
        # ↓ KANBlock 接受 [B, N, C]
        x_k = x.flatten(2).transpose(1, 2)  # [B, C, H, W] → [B, N, C]
        x_k = self.ln1(x_k)
        x_k = self.KAN(x_k)
        x = x + x_k.transpose(1, 2).view(B, C, H, W)  # [B, N, C] → [B, C, H, W]

        # ↓ VSSBlock 接受 [B, H, W, C]
        x_v = x.permute(0, 2, 3, 1)  # [B, C, H, W] → [B, H, W, C]
        x_v = self.ln2(x_v)
        x_v = self.VSS(x_v)
        x = x + x_v.permute(0, 3, 1, 2)  # [B, H, W, C] → [B, C, H, W]

        # ↓ MLPBlock 接受 [B, N, C]
        x_m = x.flatten(2).transpose(1, 2)  # [B, C, H, W] → [B, N, C]
        x_m = self.ln3(x_m)
        x_m = self.MLP(x_m, H, W)
        x = x + x_m.transpose(1, 2).view(B, C, H, W)  # [B, N, C] → [B, C, H, W]
