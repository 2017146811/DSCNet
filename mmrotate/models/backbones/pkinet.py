...
from .MFMamba import VSSBlock
# from .vkan import KANBlock, EFConv
...
class CAA(BaseModule):
    ...
    def __init__(
        ...
        #                          norm_cfg=None, act_cfg=None)
        self.gcsa = GCSA(dim=channels, num_heads=8, bias=True)
    ...
    def forward(self, x):
        ...
        # # 再还原回 [B, C, H, W]
        # x = x.permute(0, 3, 1, 2)
        # attn_factor = self.act(self.gcsa(x))
        # return x * attn_factor + x
        # attn_factor = self.act(self.conv2(self.v_conv(self.h_conv(self.conv1(self.avg_pool(x))))))
        # return attn_factor
        # identity = x  # 保存残差

        x_conv = self.conv1(x)  # 前处理
        x_attn = self.gcsa(x_conv)  # 空间建模
        # attn_mask = self.act(self.conv2(x_attn))  # [B, C, H, W] ∈ (0, 1)

        # # 残差 + 门控方式融合
        # out = x_attn * attn_mask + identity * (1 - attn_mask)
        return x_attn
...
class GCSA(nn.Module):
    def __init__(self, dim, num_heads, bias):
        ...
        # 分支 1：DWConv + PWConv
        self.qkv_dwconv1 = nn.Sequential(
            nn.Conv2d(dim * 3, dim * 3, kernel_size=3, padding=(3-1)//2, groups=dim * 3, bias=bias),  # DWConv
            nn.Conv2d(dim * 3, dim * 3, kernel_size=1, bias=bias),  # PWConv
            nn.BatchNorm2d(dim * 3),
            nn.ReLU(inplace=True)
        )
        self.project1 = nn.Conv2d(dim, dim, kernel_size=1, bias=bias)
        self.temp1 = nn.Parameter(torch.ones(num_heads, 1, 1))

        # 分支 2：DWConv + PWConv
        self.qkv_dwconv2 = nn.Sequential(
            nn.Conv2d(dim * 3, dim * 3, kernel_size=5,  padding=(5-1)//2, stride=2, groups=dim * 3, bias=bias),
            nn.Conv2d(dim * 3, dim * 3, kernel_size=1, bias=bias),
            nn.BatchNorm2d(dim * 3),
            nn.ReLU(inplace=True)
        )
        self.project2 = nn.Conv2d(dim, dim, kernel_size=1, bias=bias)
        self.temp2 = nn.Parameter(torch.ones(num_heads, 1, 1))

        # 分支 3：DWConv + PWConv
        self.qkv_dwconv3 = nn.Sequential(
            nn.Conv2d(dim * 3, dim * 3, kernel_size=7,  padding=(7-1)//2, stride=4, groups=dim * 3, bias=bias),
            nn.Conv2d(dim * 3, dim * 3, kernel_size=1, bias=bias),
            nn.BatchNorm2d(dim * 3),
            nn.ReLU(inplace=True)
        )
        self.project3 = nn.Conv2d(dim, dim, kernel_size=1, bias=bias)
        self.temp3 = nn.Parameter(torch.ones(num_heads, 1, 1))

        # 加权融合权重
        self.alpha = nn.Parameter(torch.ones(3))  # 可学习融合权重 [3]

    def forward_branch(self, qkv, conv, project, h, w, temperature):
        qkv = conv(qkv)
    ...
    def forward(self, x):
        ...
        out1 = self.forward_branch(qkv, self.qkv_dwconv1, self.project1, h, w, self.temp1)

        qkv2 = self.qkv_dwconv2(qkv)
        h2, w2 = qkv2.shape[2], qkv2.shape[3]
        out2 = self.forward_branch(qkv2, nn.Identity(), self.project2, h2, w2, self.temp2)
        out2 = torch.nn.functional.interpolate(out2, size=(h, w), mode="bilinear", align_corners=False)

        qkv3 = self.qkv_dwconv3(qkv)
        h3, w3 = qkv3.shape[2], qkv3.shape[3]
        out3 = self.forward_branch(qkv3, nn.Identity(), self.project3, h3, w3, self.temp3)
        out3 = torch.nn.functional.interpolate(out3, size=(h, w), mode="bilinear", align_corners=False)

        # 使用 softmax 加权融合三个输出
        weights = torch.softmax(self.alpha, dim=0)  # (3,)
        out = out1 * weights[0] + out2 * weights[1] + out3 * weights[2]  # 加权融合 (B, C, H, W)
...
#         # 分支 1：普通卷积
#         self.qkv_dwconv1 = nn.Conv2d(dim * 3, dim * 3, kernel_size=3, padding=1, groups=dim * 3, bias=bias)
#         self.project1 = nn.Conv2d(dim, dim, kernel_size=1, bias=bias)

#         # 分支 2：空洞卷积
#         self.qkv_dwconv2 = nn.Conv2d(dim * 3, dim * 3, kernel_size=3, dilation=2, padding=2, groups=dim * 3, bias=bias)
#         self.project2 = nn.Conv2d(dim, dim, kernel_size=1, bias=bias)

#         # 分支3
#         self.qkv_dwconv3 = nn.Conv2d(dim * 3, dim * 3, kernel_size=3, dilation=3, padding=3, groups=dim * 3, bias=bias)
#         self.project3 = nn.Conv2d(dim, dim, kernel_size=1, bias=bias)
...
#     def forward_branch(self, qkv, conv, project, h, w):
#         qkv = conv(qkv)
#         q, k, v = qkv.chunk(3, dim=1)
#         q = rearrange(q, 'b (head c) h w -> b head c (h w)', head=self.num_heads)
#         k = rearrange(k, 'b (head c) h w -> b head c (h w)', head=self.num_heads)
...
#         out1 = self.forward_branch(qkv, self.qkv_dwconv1, self.project1, h, w)
#         out2 = self.forward_branch(qkv, self.qkv_dwconv2, self.project2, h, w)
#         out3 = self.forward_branch(qkv, self.qkv_dwconv3, self.project3, h, w)

#         # 融合两个分支的注意力输出
#         out = torch.cat([out1, out2 , out3], dim=1)  # (B, 2C, H, W)
...
class MLPBlock(nn.Module):
    ...
    def forward(self, x, H=None, W=None):
        # x: [B, H, W, C] or [B, N, C]
        x_norm = self.norm(x)
        x_mlp = self.drop1(self.act(self.fc1(x_norm)))
        x_mlp = self.drop2(self.fc2(x_mlp))
        return x + x_mlp  # 残差连接
...
class InceptionBottleneck(BaseModule):
    ...
    def __init__(
        ...
        # self.EFConv = EFConv(kernel_size=3, stride=1, in_chans=hidden_channels, embed_dim=hidden_channels)
        self.drop_path = DropPath(drop_path)

        self.vss = VSSBlock(hidden_dim=hidden_channels, drop_path=drop_path)

        self.MLP = MLPBlock(dim=hidden_channels, drop=drop_rate)

        self.gate_vss = nn.Sequential(
        ...
        if with_caa:
            self.caa_factor = CAA(hidden_channels, num_heads=8, bias=True)
        else:
            self.caa_factor = None

        self.add_identity = add_identity and in_channels == out_channels

        self.fuse_conv = nn.Conv2d(hidden_channels * 2, hidden_channels, kernel_size=1)
...
#             y = self.caa_factor(y)  # CAA特征增强
#         x = x * y  # 若不添加残差，则直接相乘
#
#         x = self.post_conv(x)  # 输出通道调整
#         return x
    def forward(self, x):
        B, C, H, W = x.shape
        y = x
        ...
        # VSS
        x_v = self.vss(self.ln(x.permute(0, 2, 3, 1)))
        x_v = x_v.permute(0, 3, 1, 2)
        x = x + self.gate_vss(x) * x_v


        x_ln = self.ln(x.permute(0, 2, 3, 1))  # [B, H, W, C]
        x_mlp = self.MLP(x_ln)  # 返回也是 [B, H, W, C]
        x_mlp = x_mlp.permute(0, 3, 1, 2)  # 转成 [B, C, H, W]

        if self.caa_factor is not None:
            y = self.caa_factor(y)

        if self.add_identity:
            x_fused = torch.cat([x, y], dim=1)
            out = self.fuse_conv(x_fused)
        else:
            x = x * y
