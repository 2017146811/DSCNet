...
class HSCANS(nn.Module):
    def __init__(self, size=16, dim=2, scan_type='scan', device=None):
        super().__init__()
        self.size = int(size)  # 显式定义 size 作为实例属性
        self.dim = dim
        self.scan_type = scan_type
        self.device = device if device is not None else torch.device("cpu")
        max_num = self.size ** self.dim
        indexes = np.arange(max_num)

        if 'sweep' == scan_type:
            locs_flat = indexes
        elif 'scan' == scan_type:
            indexes = indexes.reshape(self.size, self.size)
            for i in np.arange(1, self.size, step=2):
                indexes[i, :] = indexes[i, :][::-1]
            locs_flat = indexes.reshape(-1)
        elif 'zorder' == scan_type:
            zi = ZOrderIndexer((0, self.size - 1), (0, self.size - 1))
            locs_flat = []
            for z in indexes:
                r, c = zi.rc(int(z))
                locs_flat.append(c * self.size + r)
            ...
            for i in range(2 * self.size - 1):
                if i % 2 == 0:
                    ...
                    end_col = min(i, self.size - 1)
                    for j in range(start_col, end_col + 1):
                        locs_flat.append(indexes[i - j, j])
                else:
                    start_row = max(0, i - self.size + 1)
                    end_row = min(i, self.size - 1)
                    for j in range(start_row, end_row + 1):
                        locs_flat.append(indexes[j, i - j])
            locs_flat = np.array(locs_flat)
        elif 'hilbert' == scan_type:
            bit = int(math.log2(self.size))
        ...
        index_flat = torch.LongTensor(locs_flat.astype(np.int64)).unsqueeze(0).unsqueeze(1).to(self.device)
        index_flat_inv = torch.LongTensor(locs_flat_inv.astype(np.int64)).unsqueeze(0).unsqueeze(1).to(self.device)
    ...
    def encode(self, img):
        # 调试信息
        img_encode = torch.zeros(img.shape, dtype=img.dtype, device=img.device).scatter_(2, self.index_flat_inv.to(img.device).expand(img.shape), img)
        return img_encode

    def decode(self, img):
        img_decode = torch.zeros(img.shape, dtype=img.dtype, device=img.device).scatter_(2, self.index_flat.to(img.device).expand(img.shape), img)
        return img_decode

    def to(self, device):
        """重写 to 方法，确保缓冲区张量随模型移动"""
        self.device = device
        self.index_flat = self.index_flat.to(device)
        self.index_flat_inv = self.index_flat_inv.to(device)
...
class SS2D(nn.Module):
    def __init__(
        ...
        self.scans = HSCANS(size=size, scan_type=scan_type, device=self.device)
    ...
    def forward(self, x: torch.Tensor, **kwargs):
        ...
        dynamic_size = max(H, W)  # 或 int(2 ** math.ceil(math.log2(max(H, W))))
        device = x.device
        if not hasattr(self, 'scans') or self.scans.size != dynamic_size:
            self.scans = HSCANS(size=dynamic_size, scan_type=self.scans.scan_type if hasattr(self, 'scans') else 'scan')
