...
class CAA(BaseModule):
    ...
    def __init__(
        ...
        # 替换为 ARConv 版本
        self.ar_conv = ARConv(channels, channels,
    ...
    def forward(self, x,epoch=0):
        ...
        x = self.avg_pool(x)
        x = self.conv1(x)
        x = self.ar_conv(x, epoch=epoch)
        x = self.conv2(x)
        attn_factor = self.act(x)
        return attn_factor

class CAAVSS(BaseModule):
    def __init__(self, channels: int, drop_path: float = 0.):
        super().__init__()
        self.ln = nn.LayerNorm(channels)
        self.vss = VSSBlock(hidden_dim=channels, drop_path=drop_path)

    def forward(self, x):
        # BCHW -> BHWC（VSSBlock 期望）
        x = x.permute(0, 2, 3, 1).contiguous()
        x = self.vss(self.ln(x))
        # BHWC -> BCHW
...
class ConvFFN(BaseModule):
    ...
    def __init__(
        ...
        self.ffn_layers = nn.Sequential(
            ...
            #mamba
            VSSBlock(hidden_dim=hidden_channels, drop_path=dropout_rate),
...
class InceptionBottleneck(BaseModule):
    ...
    def __init__(
        ...
        if with_caa:
            # CAA和CAAVSS串联
            self.caa_factor = nn.Sequential(
                CAA(hidden_channels, caa_kernel_size, caa_kernel_size, None, None),
                CAAVSS(hidden_channels, drop_path=0.1)
            )
        else:
    ...
    def forward(self, x,epoch=0):
        ...
        if self.caa_factor is not None:
            y = self.caa_factor(y,epoch=epoch)
...
class PKIBlock(BaseModule):
    ...
    def forward(self, x,epoch=0):
        if self.layer_scale:
            if self.add_identity:
                x = x + self.drop_path(self.gamma1.unsqueeze(-1).unsqueeze(-1) * self.block(self.norm1(x),epoch=epoch))
                x = x + self.drop_path(self.gamma2.unsqueeze(-1).unsqueeze(-1) * self.ffn(self.norm2(x)))
            else:
                x = self.drop_path(self.gamma1.unsqueeze(-1).unsqueeze(-1) * self.block(self.norm1(x),epoch=epoch))
                x = self.drop_path(self.gamma2.unsqueeze(-1).unsqueeze(-1) * self.ffn(self.norm2(x)))
        else:
            if self.add_identity:
                x = x + self.drop_path(self.block(self.norm1(x),epoch=epoch))
                x = x + self.drop_path(self.ffn(self.norm2(x)))
            else:
                x = self.drop_path(self.block(self.norm1(x),epoch=epoch))
